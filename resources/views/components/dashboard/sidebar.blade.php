                    ...
                    <ul x-show="open" class="ml-8 space-y-1 mt-2" x-cloak>
                        @can('inventoryitemsmenu')
                            <li><a href="{{ route('items.index') }}" onclick="showFullScreenLoader();"
                                    class="{{ request()->routeIs('items.index', 'items.create', 'items.edit') ? 'bg-purple-100 text-purple-700' : 'text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700' }} block py-1 px-2 rounded text-sm">Inventory
                                    Items</a></li>
                        @endcan
                        @can('equipmentsmenu')
