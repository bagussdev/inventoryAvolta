<x-app-layout>
    <x-dashboard.sidebar>
        ...
        <div class="flex justify-between items-center mt-4 w-full max-w-full">
            <h2 class="font-bold text-xl sm:text-2xl">Desktop Detail</h2>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md text-sm text-gray-700 dark:text-gray-300">
            ...
            <div class="block md:hidden space-y-4">
                <div class="grid grid-cols-1 gap-3">
                    <div class="flex">
                        ...
                        <div class="flex-1">: {{ $desktop->hostname }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Serial Number</div>
                        <div class="flex-1">: {{ $desktop->serialnumber ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Model</div>
                        <div class="flex-1">: {{ $desktop->model ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Brand</div>
                        <div class="flex-1">: {{ $desktop->brand ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">User</div>
                        <div class="flex-1">: {{ $desktop->user ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Location</div>
                        <div class="flex-1">: {{ $desktop->store->name ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Windows</div>
                        <div class="flex-1">: {{ $desktop->typewindows ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">OS Status</div>
                        <div class="flex-1">: {{ $desktop->osstatus ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">IP RealVNC</div>
                        <div class="flex-1">: {{ $desktop->iprealvnc ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Created By</div>
                        <div class="flex-1">: {{ $desktop->creator->name ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        ...
                        <div class="flex-1">:
                            <span
                                ...
                                {{ ucfirst(str_replace('_', ' ', $desktop->status)) }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="hidden md:grid grid-cols-1 md:grid-cols-3 gap-6">
                <div><label class="block text-sm font-medium mb-1">Hostname</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->hostname }}</div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Serial Number</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->serialnumber ?? '-' }}
                    </div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Model</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->model ?? '-' }}</div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Brand</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->brand ?? '-' }}</div>
                </div>
                <div><label class="block text-sm font-medium mb-1">User</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->user ?? '-' }}</div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Location</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->store->name ?? '-' }}
                    </div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Windows</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->typewindows ?? '-' }}
                    </div>
                </div>
                <div><label class="block text-sm font-medium mb-1">OS Status</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->osstatus ?? '-' }}
                    </div>
                </div>
                <div><label class="block text-sm font-medium mb-1">IP RealVNC</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->iprealvnc ?? '-' }}
                    </div>
                </div>
                <div><label class="block text-sm font-medium mb-1">Created By</label>
                    <div class="px-3 py-2 bg-gray-100 dark:bg-gray-700 rounded-md">{{ $desktop->creator->name ?? '-' }}
                    </div>
                </div>
                <div>
