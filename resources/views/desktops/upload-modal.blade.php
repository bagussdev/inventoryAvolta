<div id="uploadModalDesktop" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto
        ...
        x-data="{
            ...
            submit() {
                ...
                form.method = 'POST';
                form.action = '{{ route('desktops.import.save') }}';
        ...
        <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Upload Excel Desktop</h2>
            ...
            <div class="overflow-auto max-h-96">
                <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-600 text-sm">
                    <thead class="bg-gray-100 dark:bg-gray-700">
                        <tr>
                            <template x-for="(cell, index) in previewData[0]" :key="index">
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-3 mt-4">
            <a href="{{ route('desktops.template.download') }}"
                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm w-full sm:w-auto text-center">
                Download Template
            </a>

            <button type="button" onclick="document.getElementById('uploadModalDesktop').classList.add('hidden')"
                class="px-4 py-2 bg-gray-400 hover:bg-gray-500 text-white rounded-md text-sm w-full sm:w-auto">
                Cancel
            </button>

            <button x-show="previewData.length"
                @click="
                        if(confirmAndLoad('Apakah Anda yakin ingin menyimpan data Desktop dari Excel ini ke database?')) {
                            submit()
                        }
                    "
