<x-app-layout>
    <x-dashboard.sidebar>
        ...
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md text-sm text-gray-700 dark:text-gray-300">
            <form method="POST" action="{{ route('equipments.migrate', $equipment->id) }}"
                onsubmit="return confirmAndLoad('Are you sure to migrate?')">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex">
                        <div class="w-40 font-medium">Category</div>
                        <div class="flex-1">: {{ ucfirst($equipment->item->type ?? '-') }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Name</div>
                        <div class="flex-1">: {{ $equipment->item->name ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Model</div>
                        <div class="flex-1">: {{ $equipment->item->model ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Merk</div>
                        <div class="flex-1">: {{ $equipment->item->brand ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">S/N</div>
                        <div class="flex-1">: {{ $equipment->serial_number ?? '-' }}</div>
                    </div>
                    <div class="flex">
                        <div class="w-40 font-medium">Qty</div>
                        <div class="flex-1">: {{ $equipment->transaction->qty ?? '-' }}</div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-white mb-1">Migrate
                            From</label>
                        <input type="text" readonly value="{{ $equipment->store->name ?? 'Storage' }}"
                            class="w-full px-3 py-2 bg-gray-100 border rounded-md dark:bg-gray-700 dark:text-white">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-white mb-1">To</label>
