<x-app-layout>
    <x-dashboard.sidebar>
        ...
        <div
            ...
            {{-- Title --}}
            <div class="mb-6">
                <h2 class="text-xl sm:text-2xl font-bold text-center text-gray-900 dark:text-white">Edit Incident</h2>
            </div>

            {{-- Form Start --}}
            <form method="POST" action="{{ route('incidents.update', $incident->id) }}" enctype="multipart/form-data"
                class="space-y-6" onsubmit="return confirmAndLoad('Are you sure to update?')">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" readonly value="{{ $incident->store->name ?? '-' }}"
                        ...
                        <div>
                            ...
                            <input type="text" readonly value="{{ $incident->department->name ?? '-' }}"
                                class="w-full rounded-md border-gray-300 bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                        </div>
                    ...
                    {{-- RIGHT SIDE --}}
                    <div>
                        <label class="block font-medium mb-1">Attachment (old)</label>
                        @if ($incident->attachment_user && Storage::disk('public')->exists($incident->attachment_user))
                            @php $ext = pathinfo($incident->attachment_user, PATHINFO_EXTENSION); @endphp
                            @if (in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'gif']))
                                <img src="{{ asset('storage/' . $incident->attachment_user) }}"
                                    class="rounded max-h-48 w-full object-contain" />
                            @elseif(in_array(strtolower($ext), ['mp4', 'mov', 'avi']))
                                <video src="{{ asset('storage/' . $incident->attachment_user) }}"
