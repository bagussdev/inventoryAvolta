    ...
    <x-dashboard.sidebar>
        ...
        <div class="flex justify-center px-4 sm:px-6 lg:px-8">
            <div class="w-full max-w-4xl bg-white dark:bg-gray-800 p-6 md:p-8 rounded-lg shadow-md">
                ...
                {{-- Form --}}
                <form action="{{ route('laptops.store') }}" method="POST" class="space-y-6"
                    onsubmit="showFullScreenLoader(); return confirm('Are you sure to create this laptop?')">
                    @csrf
                            ...
                            <select id="location" name="location" required
                                ...
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id }}"
                                        {{ old('location') == $store->name ? 'selected' : '' }}>
                                        {{ $store->name }}
                                    </option>
                                @endforeach
                            </select>
