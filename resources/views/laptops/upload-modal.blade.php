<div id="uploadModalLaptop" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto
        ...
        x-data="{
            ...
            submit() {
                ...
                form.method = 'POST';
                form.action = '{{ route('laptops.import.save') }}';
        ...
        <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Upload Excel Laptop</h2>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-3 mt-4">
            <a href="{{ route('laptops.template.download') }}"
                class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm w-full sm:w-auto text-center">
                Download Template
            </a>

            <button type="button" onclick="document.getElementById('uploadModalLaptop').classList.add('hidden')"
            ...
            <button x-show="previewData.length"
                @click="
                        if(confirmAndLoad('Apakah Anda yakin ingin menyimpan data Laptop dari Excel ini ke database?')) {
                            submit()
                        }
                    "
