    ...
    <x-dashboard.sidebar>
        ...
        <div class="flex justify-center mt-6 px-4 sm:px-6 lg:px-8">
            <div class="w-full max-w-2xl bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-lg shadow-md">
                <h1 class="text-2xl sm:text-3xl font-bold mb-6 text-center text-gray-900 dark:text-white">Edit User</h1>

                <form method="POST" action="{{ route('users.update', $user->id) }}" class="space-y-5"
                    onsubmit="return confirmAndLoad('Are you sure to update this user?')">
                    @csrf
                    @method('PUT')
                        ...
                        <select id="store_location" name="store_location"
                            ...
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}"
                                    {{ $user->store_location == $store->id ? 'selected' : '' }}>
