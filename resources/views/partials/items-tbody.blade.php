<tbody id="items-body" class="list whitespace-nowrap">
            ...
            <td class="px-2 py-2 md:px-4 md:py-3">
                <div class="flex flex-row items-center justify-center gap-1">
                    @can('inventoryitems.edit')
                        <x-buttons.action-button text="Edit" color="blue" href="{{ route('items.edit', $item->id) }}"
                            onclick="showFullScreenLoader();" />
                    @endcan

                    @can('inventoryitems.delete')
                        <form action="{{ route('items.destroy', $item->id) }}" method="POST"
                            onsubmit="return confirmAndLoad('Are you sure to deleted?')" class="inline-block">
                            @csrf
                            @method('DELETE')
