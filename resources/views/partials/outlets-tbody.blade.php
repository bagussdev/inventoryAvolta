                    ...
                    @can('outletcontrol')
                        @if ($store->status == 'Y')
                            <x-buttons.action-button text="Non-Active" color="red"
                                href="{{ route('outlets.deactive', $store->id) }}" onclick="showFullScreenLoader();" />
                        @else
                            <x-buttons.action-button text="Active" color="green"
                                href="{{ route('outlets.active', $store->id) }}" onclick="showFullScreenLoader();" />
                        @endif
                    @endcan

                    @can('outletlist.edit')
                        <x-buttons.action-button text="Edit" color="blue"
                            href="{{ route('outlets.edit', $store->id) }}" onclick="showFullScreenLoader();" />
                    @endcan

                    <x-buttons.action-button text="Detail" color="purple"
                        href="{{ route('outlets.show', $store->id) }}" onclick="showFullScreenLoader();" />
                </div>
            </td>
