    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-95 opacity-0"
        x-data="{
            ...
            submit() {
                ...
                form.method = 'POST';
                form.action = '{{ route('transactions.import.save') }}';
                ...
                zipInput.type = 'file';
                zipInput.name = 'compressed_file';
                zipInput.files = document.getElementById('compressed_file').files;
        ...
        <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Upload Photo and Attachment File</h2>

        <input type="file" name="compressed_file" id="compressed_file" accept=".zip,.rar,.7z"
            class="mb-4 block w-full border rounded px-3 py-2 text-sm" required>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-3 mt-4">
            <a href="{{ route('transactions.template.download') }}"
